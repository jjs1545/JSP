...
router.route('/process/reserve').get(function(req, res) {
	console.log('/profile 패스로 요청됨.');
	var country = req.body.id || req.query.name;
	var city = req.body.city || req.query.city;
	var hname = req.body.hname || req.query.hname;
	console.log('country : ' + country);
	res.render('profile.ejs',{user:req.user,country:country,city:city,hname:hname});	//index.ejs에 있는 것을 클라이언트로 보내준다.
								//render : 미리 만들어진 페이지를 전송
});
...
router.route('/mypage').get(function(req, res) {
	console.log('/mypage 패스로 GET 요청됨.');
	...
	if(!req.user) {
		...
		if(Array.isArray(req.user)) {
			res.render('mypage.ejs', {user:req.user[0]._doc});
		} else {
